...
import me.сс.zerotwo.client.modules.misc.NoSoundLag;
...
class AutoCrystal
    ...
    void onDisable ( ) {
        ...
        Command.sendMessage(TextUtil.BLACK + "ZeroTwo on sex");
    }
    ...
    void onEnable ( ) {
        ...
        Command.sendMessage(TextUtil.WHITE + "ZeroTwo off sex");
    }
            ...
            if ( packet2.getType ( ) == 51 && AutoCrystal.mc.player.getDistanceSq ( pos = new BlockPos ( packet2.getX ( ) , packet2.getY ( ) , packet2.getZ ( ) ) ) + (double) this.predictOffset.getValue ( ) <= MathUtil.square ( this.breakRange.getValue ( ) ) && ( this.instantTimer.getValue ( ) == PredictTimer.NONE || this.instantTimer.getValue ( ) == PredictTimer.BREAK && this.breakTimer.passedMs ( this.breakDelay.getValue ( ) ) || this.instantTimer.getValue ( ) == PredictTimer.PREDICT && this.predictTimer.passedMs ( this.predictDelay.getValue ( ) ) ) ) {
                ...
                if ( this.predictFriendDmg.getValue ( ) && ( this.antiFriendPop.getValue ( ) == AntiFriendPop.BREAK || this.antiFriendPop.getValue ( ) == AntiFriendPop.ALL ) && this.isRightThread ( ) ) {
                    for (EntityPlayer friend : AutoCrystal.mc.world.playerEntities) {
                        if ( friend == null || AutoCrystal.mc.player.equals ( friend ) || friend.getDistanceSq ( pos ) > MathUtil.square ( this.range.getValue ( ) + this.placeRange.getValue ( ) ) || ! ZeroTwo.friendManager.isFriend ( friend ) || ! ( (double) DamageUtil.calculateDamage ( pos , friend ) > (double) EntityUtil.getHealth ( friend ) + 0.5 ) )
                            continue;
                        return;
                    }
                }
    ...
    boolean isRightThread ( ) {
        return mc.isCallingFromMinecraftThread ( ) || ! ZeroTwo.eventManager.ticksOngoing ( ) && ! this.threadOngoing.get ( );
    }
        ...
        for (Entity entity : AutoCrystal.mc.world.loadedEntityList) {
            ...
            if ( (double) selfDamage + 0.5 < (double) EntityUtil.getHealth ( AutoCrystal.mc.player ) && selfDamage <= this.maxSelfBreak.getValue ( ) ) {
                ...
                for (EntityPlayer player : AutoCrystal.mc.world.playerEntities) {
                    ...
                    if ( this.antiFriendPop.getValue ( ) != AntiFriendPop.BREAK && this.antiFriendPop.getValue ( ) != AntiFriendPop.ALL || ! ZeroTwo.friendManager.isFriend ( player.getName ( ) ) || ! ( (double) ( damage = DamageUtil.calculateDamage ( entity , player ) ) > (double) EntityUtil.getHealth ( player ) + 0.5 ) )
                        continue;
        ...
        for (BlockPos pos : BlockUtil.possiblePlacePositions ( this.placeRange.getValue ( ) , this.antiSurround.getValue ( ) , this.oneDot15.getValue ( ) )) {
            ...
            if ( targettedPlayer != null ) {
                ...
                if ( this.calcEvenIfNoDamage.getValue ( ) && ( this.antiFriendPop.getValue ( ) == AntiFriendPop.ALL || this.antiFriendPop.getValue ( ) == AntiFriendPop.PLACE ) ) {
                    ...
                    for (EntityPlayer friend : AutoCrystal.mc.world.playerEntities) {
                        float friendDamage;
                        if ( friend == null || AutoCrystal.mc.player.equals ( friend ) || friend.getDistanceSq ( pos ) > MathUtil.square ( this.range.getValue ( ) + this.placeRange.getValue ( ) ) || ! ZeroTwo.friendManager.isFriend ( friend ) || ! ( (double) ( friendDamage = DamageUtil.calculateDamage ( pos , friend ) ) > (double) EntityUtil.getHealth ( friend ) + 0.5 ) )
                            continue;
                        friendPop = true;
                        break;
                    }
    ...
    class RAutoCrystal
        ...
        public static
        RAutoCrystal getInstance ( AutoCrystal zeroAura) {
            if ( instance == null ) {
                instance = new RAutoCrystal ( );
                RAutoCrystal.instance.zeroAura = zeroAura;
            }
            return instance;
        }
        ...
        public
        void run ( ) {
            if ( this.zeroAura.threadMode.getValue ( ) == ThreadMode.WHILE ) {
                while ( this.zeroAura.isOn ( ) && this.zeroAura.threadMode.getValue ( ) == ThreadMode.WHILE ) {
                    while ( ZeroTwo.eventManager.ticksOngoing ( ) ) {
                    }
                    if ( this.zeroAura.shouldInterrupt.get ( ) ) {
                        this.zeroAura.shouldInterrupt.set ( false );
                        this.zeroAura.syncroTimer.reset ( );
                        this.zeroAura.thread.interrupt ( );
                        break;
                    }
                    this.zeroAura.threadOngoing.set ( true );
                    ZeroTwo.safetyManager.doSafetyCheck ( );
                    this.zeroAura.doAutoCrystal ( );
                    this.zeroAura.threadOngoing.set ( false );
                    try {
                        Thread.sleep ( this.zeroAura.threadDelay.getValue ( ) );
                    } catch ( InterruptedException e ) {
                        this.zeroAura.thread.interrupt ( );
                        e.printStackTrace ( );
                    }
                }
            } else if ( this.zeroAura.threadMode.getValue ( ) != ThreadMode.NONE && this.zeroAura.isOn ( ) ) {
                while ( ZeroTwo.eventManager.ticksOngoing ( ) ) {
                }
                this.zeroAura.threadOngoing.set ( true );
                ZeroTwo.safetyManager.doSafetyCheck ( );
