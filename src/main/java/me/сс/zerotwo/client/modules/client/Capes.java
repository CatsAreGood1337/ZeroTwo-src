...
public class Capes
    ...
    public Capes() {
        super("Capes", "Capes with superior physics", Module.Category.CLIENT, true, false, false);
    }
    ...
    public void onPlayerRender(RenderPlayerEvent.Post event) {
        ...
        GL11.glTranslated((double)0.0, (double)(-((double)event.getEntityPlayer().height - (event.getEntityPlayer().isSneaking() ? 0.25 : 0.0) - 0.38)), (double)0.0);
        ...
        if (Capes.mc.player.moveForward != 0.0f || Capes.mc.player.moveStrafing != 0.0f) {
            for (ModelRenderer renderer : this.cape.boxList) {
                renderer.rotateAngleX = ((Float)waveMap.get(renderer)).floatValue();
            }
        } else {
            for (ModelRenderer renderer : this.cape.boxList) {
