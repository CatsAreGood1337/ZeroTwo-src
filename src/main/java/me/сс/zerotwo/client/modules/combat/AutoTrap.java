...
class AutoTrap extends Module {
    ...
    EntityPlayer getTarget ( double range , boolean trapped ) {
        ...
        for (EntityPlayer player : AutoTrap.mc.world.playerEntities) {
            if ( EntityUtil.isntValid ( player , range ) || trapped && EntityUtil.isTrapped ( player , this.antiScaffold.getValue ( ) , this.antiStep.getValue ( ) , false , false , false , true ) || ZeroTwo.speedManager.getPlayerSpeed ( player ) > 10.0 )
                continue;
            if ( target == null ) {
                target = player;
