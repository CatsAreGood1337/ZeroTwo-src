...
class Managers extends Module {
    ...
    public String command = ZeroTwo.getName ( );
    public int moduleListUpdates = 0;
    public boolean rainbowPrefix = true;

    public
    Managers ( ) {
        super ( "Management" , "ClientManagement" , Module.Category.CLIENT , true , false , false );
        this.setInstance ( );
    }
    ...
    public
    void onLoad ( ) {
        ZeroTwo.commandManager.setClientMessage ( this.getCommandMessage ( ) );
    }
        ...
        if ( event.getStage ( ) == 2 ) {
            if ( event.getSetting ( ) != null && this.equals ( event.getSetting ( ).getFeature ( ) ) ) {
                if ( event.getSetting ( ).equals ( this.holeThread ) ) {
                    ZeroTwo.holeManager.settingChanged ( );
                }
                ZeroTwo.commandManager.setClientMessage ( this.getCommandMessage ( ) );
            }
        }
