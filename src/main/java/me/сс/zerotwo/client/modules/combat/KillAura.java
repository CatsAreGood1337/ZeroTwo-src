...
public class KillAura extends Module {
    ...
    private void doKillaura() {
        ...
        int wait = !this.delay.getValue().booleanValue() ? 0 : (int) (DamageUtil.getCooldownByWeapon(mc.player) * (this.tps.getValue().booleanValue() ? ZeroTwo.serverManager.getTpsFactor() : 1.0F));
        ...
        if (target == null)
            return;
        if (this.rotate.getValue().booleanValue())
            ZeroTwo.rotationManager.lookAtEntity(target);
