...
class AutoArmor extends Module {
    ...
    void onKeyInput ( InputEvent.KeyInputEvent event ) {
        if ( Keyboard.getEventKeyState ( ) && ! ( mc.currentScreen instanceof zerotwoGui) && elytraBind.getValue ( ).getKey ( ) == Keyboard.getEventKey ( ) ) {
            elytraOn = ! elytraOn;
        }
    }
    ...
    void onTick ( ) {
        ...
        if ( timer.passedMs ( (int) ( delay.getValue ( ) * ( tps.getValue ( ) ? ZeroTwo.serverManager.getTpsFactor ( ) : 1 ) ) ) ) {
            if ( ! taskList.isEmpty ( ) ) {
