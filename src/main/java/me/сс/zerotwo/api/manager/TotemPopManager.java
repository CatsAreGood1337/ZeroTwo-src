...
import me.сс.zerotwo.client.modules.client.Notifications;
...
class TotemPopManager extends Client {
    ...
    void onUpdate ( ) {
        if ( notifications.totemAnnounce.passedMs ( notifications.delay.getValue ( ) ) && notifications.isOn ( ) && notifications.totemPops.getValue ( ) ) {
            for (EntityPlayer player : toAnnounce) {
                ...
                Command.sendMessage ( TextUtil.GREEN + player.getName ( ) + " popped " + TextUtil.GREEN + getTotemPops ( player ) + TextUtil.RED + " Totem" + ( getTotemPops ( player ) == 1 ? "" : "s" ) + "." , notifications.totemNoti.getValue ( ) );
    ...
    public
    void init ( ) {
        this.notifications = ZeroTwo.moduleManager.getModuleByClass ( Notifications.class );
    }
