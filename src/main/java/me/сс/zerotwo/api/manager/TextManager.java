...
import me.сс.zerotwo.client.gui.font.CustomFont;
import me.сс.zerotwo.client.modules.client.FontMod;
...
class TextManager extends Client {
    ...
    void init ( boolean startup ) {
        FontMod cFont = ZeroTwo.moduleManager.getModuleByClass ( FontMod.class );
        try {
    ...
    void drawString ( String text , float x , float y , int color , boolean shadow ) {
        if ( ZeroTwo.moduleManager.isModuleEnabled ( FontMod.class ) ) {
            if ( shadow ) {
